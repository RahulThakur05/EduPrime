...
class BioVideoScreen extends StatefulWidget {
  const BioVideoScreen({Key? key, required this.videoKey,required this.selectedVideoIndex}) : super(key: key);
  final String videoKey;
  final int selectedVideoIndex;

  @override
  _BioVideoScreenState createState() => _BioVideoScreenState();
}

class _BioVideoScreenState extends State<BioVideoScreen> {
 ...
 /* Future<void> updateLastPlayedVideo(String videoUrl, String videoTitle, String videoDescription) async {
    final apiUrl = '$globalapi/update_last_played_video/';

    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      final userEmail = prefs.getString('userEmail'); // Retrieve the stored user email from shared preferences
      print(userEmail);
      if (userEmail != null) {
        final response = await http.post(
          Uri.parse(apiUrl),
          body: jsonEncode({
            'user_email': userEmail, // Send the user's email in the request body
            'video_url': videoUrl,
...
/*  Future<void> updateLastPlayedVideo(String videoUrl, String videoTitle, String videoDescription) async {
    final apiUrl = '$globalapi/update_last_played_video/'; // Replace with your API endpoint URL
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // The server is redirecting; get the redirect URL
        final redirectUrl = response.headers['location'];
      }
      else {
        // Handle error
        print('Failed to update last played video');
      }
    } catch (e) {
      print('Error: $e');
    }
...
/*  void onVideoAccessed() {
  ...
  Future<void> fetchMostPlayedVideos() async {
    final apiUrl = '$globalapi/most_played_videos/';
  ...
  void onVideoAccessed() async {
    if (selectedVideoIndex >= 0 && selectedVideoIndex < videoUrls.length) {
      try {
        final apiUrl = '$globalapi/update_view_count/';
        final response = await http.post(
          Uri.parse(apiUrl),
          body: {
            'video_url': videoUrls[selectedVideoIndex],
          },
        );
        if (response.statusCode == 200) {
                  ...
                  itemBuilder: (context, index) {
                    return Padding(
                      ...
                      child: GestureDetector(
                        onTap: (){
                          Navigator.push(context, MaterialPageRoute(builder: (contex)=> BioVideoScreen(videoKey: videoUrls[index],selectedVideoIndex: index),));
                        },
                        child: Container(
                          width: 250,
